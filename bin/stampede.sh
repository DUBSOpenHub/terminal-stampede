...
if ! $NO_ATTACH; then
    ...
    if [[ "$(uname)" == "Darwin" ]]; then
        ...
        # Build task list for the intro
        TASK_LIST=""
        if [[ -d "${BASE_DIR}/queue" ]]; then
            while IFS= read -r line; do
                TASK_LIST="${TASK_LIST}${line}\n"
            done < <(cd "${BASE_DIR}/queue" && for qf in *.json; do
                [ -f "$qf" ] || continue
...
printf "  \${TX}F1-F${WORKER_COUNT}  Zoom an agent fullscreen\${R}\n"
printf "  \${TX}F9    Return to the grid\${R}\n\n"
sleep 1
